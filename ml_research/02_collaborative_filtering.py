...
decorative. With a generated 5,060-item Zepto catalogue, real ALS/SVD
also needs a sufficiently dense interaction matrix to factorise; Instacart's products
...
# ════════════════════════════════════════════════════════════
# Step 1 — Load the canonical catalogue generated by generate_large_catalog.py
# ════════════════════════════════════════════════════════════
import json
...
# The production-like catalogue contains thousands of items. More events per
# user keep the benchmark from measuring an almost entirely cold-item problem.
N_USERS = 1000
MIN_INTERACTIONS, MAX_INTERACTIONS = 30, 60
...
n_components = min(32, N_PRODUCTS - 1)
...
print(f"  Candidate pool per user: ~{N_PRODUCTS - MAX_INTERACTIONS}-{N_PRODUCTS - MIN_INTERACTIONS} "
    f"items; @10 is evaluated against the full catalogue minus known items.")
